  ...
  <div class="m-2">
    ...
    <template #cell(sender)="data">
        <span class="text-nowrap">
          <b-badge
          :id="`outbox-row-${data.item.id}-sender`"
          :variant="resolveOutboxSenderTypeVariant(data.item.type_label).variant"
          >
          {{ data.item.sender }}
          </b-badge>
        </span>
      ...
      <template #cell(replied)="data">
        <span class="text-nowrap">
          <b-badge
          :id="`outbox-row-${data.item.id}-replied`"
          :variant="resolveOutboxStatusVariant(data.item.replied).variant"
          >
          {{ data.item.replied }}
          </b-badge>
          <b-tooltip
      ...
      <template #cell(created_at)="data">
        <span class="text-nowrap"
        :id="`outbox-row-${data.item.id}-created-at`"
        >
          {{ formatDateToMonthLong(data.value) }}
        <b-tooltip
            title="Sent At"
            class="cursor-pointer"
            :target="`outbox-row-${data.item.id}-created-at`"
            placement="topright"
          >
          <p class="mb-0">
      ...
      <template #row-details="row">
        ...
        <b-col cols="12">
            <div>
              <span class="ml-1"><strong class="mr-1 text-muted">Link ID:</strong> {{ row.item.link_id }}</span>
                <span class="ml-2">
