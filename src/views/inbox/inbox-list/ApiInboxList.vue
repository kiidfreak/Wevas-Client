  ...
  <div class="m-2">
      ...
      <b-row>
        ...
        >
          ...
          <!-- <b-button
            variant="primary"
            @click="isAddNewInboxSidebarActive = true"
          >
            <span  class="text-nowrap">Add New Inbox</span>
          </b-button> -->
        </b-col>
    ...
    <template #cell(show_details)="row">
      ...
      <template #cell(sms_count)="data">
        <span class="text-nowrap">
          <b-badge
          :id="`inbox-row-${data.item.id}-sms-count`"
          variant="warning"
          >
          {{ data.item.sms_count }}
          </b-badge>
          <b-tooltip
            ...
            :target="`inbox-row-${data.item.id}-sms-count`"
            placement="topright"
          >
          <p class="mb-0">
      ...
      <template #cell(state)="data">
        <span class="text-nowrap">
          <b-badge
          :id="`inbox-row-${data.item.id}-state`"
          :variant="resolveInboxStatusVariant(data.item.state).variant"
          >
          {{ data.item.state }}
          </b-badge>
          <b-tooltip
      ...
      <template #cell(created_at)="data">
        <span class="text-nowrap"
        :id="`inbox-row-${data.item.id}-sent-at`"
        >
          {{ formatDateToMonthLong(data.value) }}
        <b-tooltip
            title="Sent At"
            class="cursor-pointer"
            :target="`inbox-row-${data.item.id}-sent-at`"
            placement="topright"
          >
          <p class="mb-0">
...
export default {
  ...
  setup() {
    ...
    // Register module
    if (!store.hasModule(INBOX_STORE_MODULE_NAME)) store.registerModule(INBOX_STORE_MODULE_NAME, outBoxStoreModule)
    ...
    // const isAddNewInboxSidebarActive = ref(false)
